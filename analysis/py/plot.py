...
def set_xlabel_month_by_day(ax, hide_text= False):
    if not hide_text:
...
def plot_measurements(j: juhrnal.Juhrnal):
    ...
    axarr[0].set_title('Arterial pressure by day')
    plt.xlim([x[0] - datetime.timedelta(days= 1), x[-1] + datetime.timedelta(days= 1)])
    ...
    plt.title('Arterial pressure, scatter plot')
    ...
    axarr[0].axhline(80, color= '0.8', zorder= -1.0)
